...
- **Department-targeted queue items** (§6a): accept updates/creates `pp_departments` with `PP_DEPARTMENT_EDITABLE_FIELDS`; `overview_doc` is replaced wholesale and the queue UI shows it as a collapsible block, not a diff. **Not changelog-logged**: `pp_changelog.tool_id` is a NOT NULL FK to `pp_tools`, so department history isn't recorded (spec §6a assumed it would be) — a `department_id` column on `pp_changelog` would fix that.
- **Stubs for the next pass**: `/api/cron/post-pulse-sync` (vercel.json fires it `0 12 1,15 * *`; returns `{stub:true}`) — the RSS + `web_search` classification pull with confidence-based auto-publish (spec §5).
