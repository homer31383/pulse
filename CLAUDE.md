...
- **Token caps** (`max_tokens`, per stream segment — pause_turn continuations each get a fresh cap): 12000 for briefings. The cap covers adaptive thinking + article text; 8000 truncated a long-form multi-film edition mid-sentence. Unused headroom costs nothing
- **Output guards** (`runWebSearchStream`): content under 300 chars ("no article text") or `stop_reason === 'max_tokens'` ("truncated at the max_tokens cap") throws instead of persisting a stub/cut-off briefing — live routes surface the error, the cron's hourly catch-up regenerates. A truncated briefing is worse than none because nothing flags it as broken
