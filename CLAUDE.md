...
- **Research/chat gotcha — resuming after a client tool call in a search turn**: when a round used `web_search_20260209` and ended with OUR `tool_use` (chat's `propose_change`), sending the tool_result back fails with 400 "container_id is required when there are pending tool uses generated by code execution with tools", and the response carries **no** container id to return (probed Sept 14 2026: `message.container` is null). Fix in `runChatTurn`: replay the assistant turn through `stripServerToolBlocks()` (drop `server_tool_use`, `web_search_tool_result`, `code_execution_tool_result`; keep thinking/text/tool_use) and run the rest of that turn with the proposal tool only — with search still offered the model re-searches instead of wrapping up. `pause_turn` resumes with the full turn as before. `web_search_20260209` runs the model's searches inside a code-execution step and Sonnet batches several per step — with `max_uses` equal to the stated budget the first run burned all five searches and read nothing. The prompt now states the budget (5), demands one search at a time, and `max_uses` carries +3 headroom. The briefing-generator's "budget note is load-bearing" lesson applies here too. Two mismatches with Pulse's briefing model are deliberate, not papered over: briefings are profile-scoped (the research channel lives in one profile) and the channel is a normal channel (a manual "Generate" on it runs an ordinary web-search briefing from its instructions, not the research mechanism).
