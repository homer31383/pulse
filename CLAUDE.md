...
app/
  ...
  post-pulse/                   — Post Pulse (see section below): layout.tsx loads the pp_* dataset once
    page.tsx                    — Landing: pipeline map (four stages, post-production sub-groups, department cards)
...
components/
  ...
  post-pulse/
    ...
    PipelineMap.tsx           — Landing flowchart: stage row → in-place expansion → post-production sub-groups → department cards (one shared MapCard for both layers)
    ...
    HowToUse.tsx              — "How to use" nav entry + the guide in an overlay (compare-mode pattern); copy is verbatim from the author, do not paraphrase
...
- **Landing page = the pipeline map** (`components/post-pulse/PipelineMap.tsx`, Sept 14 2026): four stage boxes in a row (Pre-production, Production, Post-production, Finishing & delivery) with department counts, from `pp_departments.pipeline_stage` (migration 022; `pipeline_substage` only for post-production, enforced by check constraints). Clicking a stage expands it **in place** (component state, no route change, like the compare overlay). Post-production is two-level: a row of four sub-group boxes (asset_creation → performance_simulation → rendering_capture → comp_generative, `PP_PIPELINE_SUBSTAGES`), each expanding to department cards that link to `/post-pulse/departments/[slug]`. The other stages expand straight to a card grid — pre-production has one department since Sept 14 2026, production and finishing are an honest "nothing tracked here yet" empty state (0 departments is accurate, not an error). Departments with a NULL stage are listed under "Not placed on the pipeline yet" so nothing is hidden. Post-production opens by default. **Sub-groups and departments use one card** (`MapCard`, Sept 15 2026): eyebrow / title / meta in the same 2-or-4-column grid, so a stage without a sub-group layer (Finishing & delivery's four departments) reads like Post-production's sub-group row rather than as a row of pills. A sub-group card's eyebrow is its "1 of 4" counter; a department card's eyebrow is its research age ("checked 3d ago", red at ≥ 14 days) and its meta line is the tool count. The mapping for the 13 seeded departments is `supabase/post_pulse_pipeline_stages_seed_update.sql` (run after 022; `seed_post_pulse.sql` doesn't touch these columns).
...
- **Workflow docs** (spec §11, migration 030 `pp_workflow_docs`, Sept 14 2026): a chat answer saved with its exact prompt under one department. `lib/post-pulse-workflows.ts` (client-safe) generates the title from the prompt (question scaffold stripped, editable on save) and matches referenced tools by name against the department's roster + related departments. `lib/post-pulse.ts`: `createWorkflowDoc`, `listWorkflowDocs` / `getWorkflowDoc` (staleness computed **at display time** from `pp_changelog` rows newer than `coalesce(last_verified_at, created_at)` for any referenced tool — one query per list, never cached), `verifyWorkflowDoc` (last_verified_at = now), `deleteWorkflowDoc`. API: `POST /api/post-pulse/workflow-docs {sessionId, messageIndex, departmentId, title?}` derives prompt/content/sources from the stored session; `PATCH …/[id] {action:'verify'}`, `DELETE`. UI: "Save as workflow doc" under every assistant message in `ChatClient` (department select when the session is unscoped); a collapsed `<details>` disclosure (`WorkflowsDisclosure`, "Workflows · N") below the tool list on the department's main page (`/post-pulse/tools?dept=slug`, fetched by the server wrapper in `tools/page.tsx`), not on the department doc (badge only when stale; items click through); `/post-pulse/departments/[slug]/workflows/[id]` with prompt, content, referenced tools (changed ones flagged), sources, Re-run / Mark still accurate / Delete. **Re-run** = `/post-pulse/chat/start?dept=slug&rerun=<docId>` → `ChatClient` auto-sends the saved prompt once and offers "Save as new version"; the saved doc is never overwritten. **Filing** (migration 031 `also_department_ids uuid[]`): `department_id` is the primary filing (anchors the URL), `also_department_ids` the extra places; `listWorkflowDocs` matches either (falls back to primary-only until 031 is applied), the doc page is reachable under any filed department and shows "Filed under X and also Y", the disclosure marks "also filed here" rows, and `PATCH …/[id] {action:'move', departmentId, alsoDepartmentIds}` (`moveWorkflowDoc`) re-matches `referenced_tool_ids` against the new rosters; the "Change filing" editor lives in `WorkflowDocActions`. **Scope** (migration 032 `messages jsonb`): the save form offers "This answer only" (default), "The whole conversation up to here", or "From: <an earlier prompt>"; the API takes `startIndex` (a user message at or before the answer's own prompt) and stores the slice in `messages` — `prompt` becomes the slice's opening prompt (what Re-run asks again), `content` the final answer, sources the union of the slice's, referenced tools matched over every assistant message. Single-answer docs keep `messages = []`. The doc page renders a saved conversation as an ordered transcript; the disclosure marks them "conversation · N prompts". Until 032 is applied a conversation save is refused (409) naming the migration.
...
- **Sidebar vs mini player**: the sidebar is `h-screen overflow-y-auto` and the Listen Queue mini player is `fixed bottom-0 z-40`, so the sidebar's last items scroll under the player unless the sidebar carries bottom padding (`pb-28`) — the "How to use" entry was unclickable without it (Sept 14 2026). The pipeline map's department cards show "checked Nd ago" (red at ≥ 14 days) so the guide's housekeeping step is true.
