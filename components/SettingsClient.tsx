  ...
  return (
    <div className="flex items-start justify-between gap-4 py-3.5">
      <div className="flex-1">
        ...
        {description && <p className="text-xs text-ink-50 mt-0.5 leading-relaxed">{description}</p>}
      </div>
      <button
        role="switch"
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 pt-6 pb-16 space-y-10">
      ...
      <div className={`text-right text-xs transition-opacity duration-300 ${saveState !== 'idle' ? 'opacity-100' : 'opacity-0'}`}>
        <span className={saveState === 'saved' ? 'text-emerald-600' : saveState === 'error' ? 'text-red-600' : 'text-ink-50'}>
      ...
      <section className="space-y-4">
        <div>
          <h2 className="font-display text-base font-normal text-ink-300">Usage &amp; cost</h2>
        ...
        {usage ? (
          <>
            ...
            <div className="grid grid-cols-5 gap-2">
              {[
                ...
                <div
                  key={label}
                  className="bg-cream-50 border border-cream-300 rounded-xl px-3 py-3 text-center"
                >
                  <p className="text-xs text-ink-50 mb-1 truncate">{label}</p>
                  <p className="text-sm font-semibold text-ink-300 tabular-nums">
                    {formatCost(value)}
                  </p>
                </div>
              ))}
            ...
            <div className="bg-cream-50 border border-cream-300 rounded-xl p-4 space-y-2">
              <p className="text-xs font-medium text-ink-100">Daily spend — last 30 days</p>
              {usage.daily.every((d) => d.cost === 0) ? (
                <p className="text-xs text-ink-50 py-4 text-center">No usage in the last 30 days.</p>
              ) : (
                <>
            ...
            {usage.byChannel.length > 0 && (
              <div className="bg-cream-50 border border-cream-300 rounded-xl divide-y divide-cream-300">
                <p className="px-4 py-2.5 text-xs font-medium text-ink-100">By channel</p>
                {usage.byChannel.map(({ channelName, cost, calls }) => (
                  <div key={channelName} className="flex items-center justify-between px-4 py-2.5">
                    <div>
                      <p className="text-sm text-ink-200">{channelName}</p>
                      <p className="text-xs text-ink-50">{calls} call{calls !== 1 ? 's' : ''}</p>
                    </div>
                    <span className="text-sm font-medium text-ink-200 tabular-nums">
                      {formatCost(cost)}
                    </span>
                  </div>
                ))}
          ...
          <div className="bg-cream-50 border border-cream-300 rounded-xl px-4 py-8 text-center">
            <p className="text-xs text-ink-50">Loading usage data…</p>
          </div>
        )}
      </section>
      ...
      <section className="space-y-3">
        <div>
          <h2 className="font-display text-base font-normal text-ink-300">Model</h2>
          <p className="text-xs text-ink-50 mt-0.5">
            ...
            return (
              <button
                ...
                className={[
                  ...
                  selected
                    ? 'bg-brand-50 border-brand-400/60 shadow-sm shadow-brand-500/10'
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  <span className={[
                    ...
                    selected ? 'border-brand-500 bg-brand-500' : 'border-cream-400',
                  ].join(' ')}>
                    {selected && <span className="w-1.5 h-1.5 rounded-full bg-white" />}
                  </span>
                  <span className="font-medium text-sm text-ink-300">{m.label}</span>
                  <span className={[
                    'text-xs px-2 py-0.5 rounded-full font-medium',
                    selected ? 'bg-brand-100 text-brand-700' : 'bg-cream-200 text-ink-100',
                  ].join(' ')}>
                    {m.badge}
                  </span>
                </div>
                <p className="text-xs text-ink-100 pl-6">{m.description}</p>
              </button>
            )
          })}
        </div>
      ...
      <section className="space-y-3">
        <div>
          <h2 className="font-display text-base font-normal text-ink-300">Briefing depth</h2>
          <p className="text-xs text-ink-50 mt-0.5">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="font-medium text-sm text-ink-300">{d.label}</span>
                </div>
                <p className="text-xs text-ink-100 pl-6">{d.description}</p>
              </button>
            )
          })}
        </div>
      ...
      <section className="space-y-3">
        <div>
          <h2 className="font-display text-base font-normal text-ink-300">Briefing format</h2>
          <p className="text-xs text-ink-50 mt-0.5">Change how briefings are generated and presented.</p>
        </div>
      ...
      <section className="space-y-3">
        <div>
          <h2 className="font-display text-base font-normal text-ink-300">History</h2>
          <p className="text-xs text-ink-50 mt-0.5">
            Automatically delete briefings older than a set age when the app loads.
          </p>
        </div>
        <div className="bg-cream-50 border border-cream-300 rounded-2xl px-4 py-3.5 shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm text-ink-300">Auto-delete old briefings</p>
              <p className="text-xs text-ink-50 mt-0.5">Keeps storage tidy. Deletions happen on home page load.</p>
            </div>
            <select
              value={retentionDays ?? ''}
              onChange={(e) => {
              ...
              className="flex-shrink-0 bg-cream-100 border border-cream-300 rounded-xl px-3 py-2 text-sm text-ink-200 focus:outline-none focus:border-brand-500/60 cursor-pointer"
            >
        ...
        {digestMode && (
          <Link
            href="/digest-history"
            className="flex items-center justify-between w-full px-4 py-3 bg-cream-50 border border-cream-300 rounded-xl hover:border-cream-400 transition-colors"
          >
            <span className="text-sm text-ink-200">View digest history</span>
        ...
        <Link
          ...
          className="flex items-center justify-between w-full px-4 py-3 bg-cream-50 border border-cream-300 rounded-xl hover:border-cream-400 transition-colors"
        >
          <span className="text-sm text-ink-200">View weekly summary history</span>
          <svg className="w-4 h-4 text-ink-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          </svg>
        </Link>
      ...
      <section className="space-y-3">
        <div>
          <h2 className="font-display text-base font-normal text-ink-300">Content tools</h2>
          <p className="text-xs text-ink-50 mt-0.5">Interactive features that appear on briefings.</p>
        </div>
        <div className="bg-cream-50 border border-cream-300 rounded-2xl px-4 divide-y divide-cream-300 shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
          <ToggleRow
        ...
        {highlightsEnabled && (
          <Link
            ...
            className="flex items-center justify-between w-full px-4 py-3 bg-cream-50 border border-cream-300 rounded-xl hover:border-cream-400 transition-colors"
          >
            <span className="text-sm text-ink-200">View saved notes</span>
            <svg className="w-4 h-4 text-ink-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
          </Link>
      ...
      <section className="space-y-3">
        <div>
          <h2 className="font-display text-base font-normal text-ink-300">Intelligence</h2>
          <p className="text-xs text-ink-50 mt-0.5">Features that look across channels and topics.</p>
        </div>
        <div className="bg-cream-50 border border-cream-300 rounded-2xl px-4 divide-y divide-cream-300 shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
          <ToggleRow
              ...
              <div className="pb-4 space-y-3">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-cream-100 border border-cream-300 rounded-xl px-3 py-2 text-sm text-ink-300 placeholder:text-ink-50 focus:outline-none focus:border-brand-500/60"
                  />
                  <button
                    onClick={addTerm}
                ...
                {watchlistTerms.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {watchlistTerms.map((term, i) => (
                      <span
                        key={i}
                        className="flex items-center gap-1.5 bg-cream-100 border border-cream-300 text-ink-200 text-xs px-2.5 py-1 rounded-full"
                      >
                        {term}
                        <button
                          onClick={() => removeTerm(i)}
                          className="text-ink-50 hover:text-red-400 transition-colors leading-none"
                ...
                {watchlistTerms.length === 0 && (
                  <p className="text-xs text-ink-50">No terms yet. Add companies, people, or topics above.</p>
                )}
              </div>
            )}
          </div>
        ...
        <div className="bg-cream-50 border border-cream-300 rounded-2xl px-4 divide-y divide-cream-300 shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
          ...
          {ttsEnabled && (
            <div className="py-4 space-y-4">
              ...
              {/* Speed selector */}
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-ink-100">Speed</label>
                <div className="flex gap-2">
                  {TTS_SPEEDS.map((s) => (
                    <button
                      key={s}
                      ...
                      className={[
                        ...
                        ttsSpeed === s
                          ...
                          : 'bg-cream-50 border border-cream-300 text-ink-100 hover:border-cream-400',
                      ].join(' ')}
                    >
                      {s}×
                    </button>
      ...
      <section className="space-y-3">
        <div>
          <h2 className="font-display text-base font-normal text-ink-300">Delivery</h2>
          <p className="text-xs text-ink-50 mt-0.5">How and when you receive your briefings.</p>
        </div>
        <div className="bg-cream-50 border border-cream-300 rounded-2xl px-4 divide-y divide-cream-300 shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
          {/* Email */}
          <div>
            ...
            {emailEnabled && (
              <div className="pb-4 space-y-2">
                <input
                  ...
                  className="w-full bg-cream-100 border border-cream-300 rounded-xl px-3 py-2 text-sm text-ink-300 placeholder:text-ink-50 focus:outline-none focus:border-brand-500/60"
                />
            ...
            {notificationsEnabled && (
              <div className="pb-4 space-y-2">
                <div className="flex items-center gap-3">
                  <label className="text-xs text-ink-100">Reminder time</label>
                  <input
                    type="time"
